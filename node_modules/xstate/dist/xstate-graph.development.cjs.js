  ...
  constructor(testLogic, options) {
    ...
    this._toTestPath = statePath => {
      ...
      return {
        ...
        description: dist_xstateActors.isMachineSnapshot(statePath.state) ? `Reaches ${getDescription(statePath.state).trim()}: ${eventsString}` : JSON.stringify(statePath.state)
      };
    };
    this.options = {
...
function createTestModel(machine, options) {
  ...
  const testModel = new TestModel(machine, {
    ...
    events: state => {
      ...
      return dist_xstateActors.getAllOwnEventDescriptors(state).flatMap(eventType => {
...
function createDefaultMachineOptions(machine, options) {
  ...
  const traversalOptions = {
    ...
    events: state => {
      const events = typeof getEvents === 'function' ? getEvents(state) : getEvents ?? [];
      return dist_xstateActors.getAllOwnEventDescriptors(state).flatMap(type => {
...
function resolveTransitionSnapshot(result) {
...
function getAdjacencyMap(logic, options) {
  ...
  while (queue.length) {
    ...
    for (const nextEvent of events) {
      ...
      const nextSnapshot = resolveTransitionSnapshot(transition(state, nextEvent, actorScope));
...
function isMachine(value) {
  return !!value && value instanceof StateMachine.StateMachine;
}
