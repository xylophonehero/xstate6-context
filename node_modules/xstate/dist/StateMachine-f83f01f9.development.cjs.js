...
function createSpawner(actorScope, {
  ...
  const spawn = (src, options) => {
    if (typeof src === 'string') {
      const logic = dist_xstateActors.resolveReferencedActor(machine, src);
      if (!logic) {
      ...
      const actor = dist_xstateActors.createActor(src, {
        id: options?.id,
        parent: actorScope.self,
        syncSnapshot: options?.syncSnapshot,
        input: options?.input,
  ...
  return (src, options) => {
    ...
    spawnedChildren[actor.id] = actor;
    actorScope.defer(() => {
      if (actor._processingStatus === dist_xstateActors.ProcessingStatus.Stopped) {
        return;
      }
      actor.start();
    });
...
class StateNode {
  ...
  config, options) {
    ...
    this.id = this.config.id || [this.machine.id, ...this.path].join(dist_xstateActors.STATE_DELIMITER);
    ...
    this.states = this.config.states ? dist_xstateActors.mapValues(this.config.states, (stateConfig, key) => {
    ...
    this.tags = dist_xstateActors.toArray(config.tags).slice();
  ...
  /** @internal */
  _initialize() {
    this.after = dist_xstateActors.getDelayedTransitions(this);
    this.transitions = formatTransitions(this);
    if (this.type === 'choice') {
      this.always = formatChoiceTransitions(this);
    } else if (this.config.always) {
      this.always = mapTransitionConfigs(this.config.always, transition => dist_xstateActors.formatTransition(this, dist_xstateActors.NULL_EVENT, transition));
    }
  ...
  next(snapshot, event, self) {
    ...
    const candidates = memo(this, `candidates-${eventType}`, () => dist_xstateActors.getCandidates(this, eventType));
    for (const candidate of candidates) {
      const guardPassed = dist_xstateActors.evaluateCandidate(candidate, event, snapshot, this, self);
      if (guardPassed) {
        return [candidate];
      }
    }
...
function formatChoiceTransitions(stateNode) {
  ...
  return [dist_xstateActors.formatTransition(stateNode, dist_xstateActors.NULL_EVENT, {
    to: args => validateChoiceResult(choice(args))
  })];
}
function mapTransitionConfigs(transitionsConfig, mapper) {
  const transitionConfigs = dist_xstateActors.toTransitionConfigArray(transitionsConfig);
...
function formatTransitions(stateNode) {
  ...
  const formatInvokeCompletionTransition = (descriptor, transitionConfig, timeoutEventType) => {
    ...
    return dist_xstateActors.formatTransition(stateNode, descriptor, {
      ...rest,
      reenter,
      to: (args, enq) => {
        if (to) {
  ...
  for (const invokeDef of stateNode.invoke) {
    const invokeTimeoutEventType = invokeDef.timeout !== undefined ? dist_xstateActors.createInvokeTimeoutEvent(invokeDef.id).type : undefined;
    if (invokeDef.onDone) {
      const descriptor = `xstate.done.actor.${invokeDef.id}`;
      const invokeDoneTransitions = mapTransitionConfigs(invokeDef.onDone, transition => invokeTimeoutEventType ? formatInvokeCompletionTransition(descriptor, transition, invokeTimeoutEventType) : dist_xstateActors.formatTransition(stateNode, descriptor, transition));
    ...
    if (invokeDef.onError) {
      ...
      transitions.set(descriptor, mapTransitionConfigs(invokeDef.onError, transition => invokeTimeoutEventType ? formatInvokeCompletionTransition(descriptor, transition, invokeTimeoutEventType) : dist_xstateActors.formatTransition(stateNode, descriptor, transition)));
    }
    if (invokeDef.onSnapshot) {
      const descriptor = `xstate.snapshot.${invokeDef.id}`;
      transitions.set(descriptor, mapTransitionConfigs(invokeDef.onSnapshot, transition => dist_xstateActors.formatTransition(stateNode, descriptor, transition)));
    }
  }
...
class StateMachine {
  ...
  config, implementations) {
    ...
    dist_xstateActors.formatRouteTransitions(this.root);
  ...
  resolveState(config) {
    const resolvedStateValue = dist_xstateActors.resolveStateValue(this.root, config.value);
    const nodeSet = dist_xstateActors.getAllStateNodes(dist_xstateActors.getStateNodes(this.root, resolvedStateValue));
    const nodes = [...nodeSet];
    return dist_xstateActors.createMachineSnapshot({
      _nodes: nodes,
      value: resolvedStateValue,
      context: config.context || {},
      children: {},
      status: dist_xstateActors.isInFinalState(nodeSet, this.root) ? 'done' : config.status || 'active',
  ...
  getTransitionData(snapshot, event, self) {
    return dist_xstateActors.transitionNode(this.root, snapshot.value, snapshot, event, self) || [];
  }
  isInternalEventType(eventType) {
    for (const descriptor of this.internalEventDescriptors) {
      if (dist_xstateActors.matchesEventDescriptor(eventType, descriptor)) {
        return true;
      }
    }
    return false;
  ...
  _getPreInitialState(actorScope, initEvent) {
    ...
    const preInitial = dist_xstateActors.createMachineSnapshot({
  ...
  getStateNodeById(stateId) {
    const fullPath = dist_xstateActors.toStatePath(stateId);
    const relativePath = fullPath.slice(1);
    const resolvedStateId = dist_xstateActors.isStateId(fullPath[0]) ? fullPath[0].slice(STATE_IDENTIFIER.length) : fullPath[0];
    const stateNode = this.idMap.get(resolvedStateId);
    if (!stateNode) {
      throw new Error(`Child state node '#${resolvedStateId}' does not exist on machine '${this.id}'`);
    }
    return dist_xstateActors.getStateNodeByPath(stateNode, relativePath);
  }
  getPersistedSnapshot(snapshot, options) {
    return dist_xstateActors.getPersistedSnapshot(snapshot, options);
  }
  restoreSnapshot(snapshot, _actorScope) {
    ...
    for (const actorId of Object.keys(snapshotChildren)) {
      ...
      const logic = typeof src === 'string' ? dist_xstateActors.resolveReferencedActor(this, src) : src;
      if (!logic) {
        continue;
      }
      const actor = dist_xstateActors.createActor(logic, {
    ...
    const nodes = Array.from(dist_xstateActors.getAllStateNodes(dist_xstateActors.getStateNodes(this.root, snapshotData.value)));
    const {
      version: _persistedSnapshotVersion,
      ...persistedRest
    } = snapshot;
    const restoredSnapshot = dist_xstateActors.createMachineSnapshot({
      ...persistedRest,
      children,
      _nodes: nodes,
    ...
    function reviveContext(contextPart) {
      ...
      for (const key of Object.keys(contextPart)) {
        ...
        if (value && typeof value === 'object') {
          if ('xstate$$type' in value && value.xstate$$type === dist_xstateActors.$$ACTOR_TYPE) {
